...
class UserViewset(CreateModelMixin, mixins.RetrieveModelMixin, mixins.UpdateModelMixin, viewsets.GenericViewSet):
    ...
    # 前一个在header添加token后一个在浏览器添加session, BasicAuthentication默认弹出登录页面
    authentication_classes = (JSONWebTokenAuthentication, authentication.SessionAuthentication)

    def create(self, request, *args, **kwargs):
        # get_serializer  Return the serializer instance that should be used for validating
        serializer = self.get_serializer(data=request.data)
        serializer.is_valid(raise_exception=True)
    ...
    # 只有使用viewset才有action放在self中这个好处， 如果使用Apiview没有这个好处的
    ...
    # 重写get_object方法，就知道是哪个用户了，控制Retrieve,update （创建一个instance）
    def get_object(self):
